...
| <a name="input_image_pull_secret_name"></a> [image\_pull\_secret\_name](#input\_image\_pull\_secret\_name) | Name of an existing docker-registry Secret in the namespace for pulling a private operator image (set on the operator ServiceAccount via the chart's imagePullSecrets). Empty for a public image. | `string` | `""` | no |
