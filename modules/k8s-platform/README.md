...
| <a name="input_image_pull_secret_name"></a> [image\_pull\_secret\_name](#input\_image\_pull\_secret\_name) | Name of a Kubernetes docker-registry secret in the operator namespace used to pull the (private) operator image. Empty when the image needs no pull secret. The greenfield/root composition creates the secret and passes its name here. | `string` | `""` | no |
